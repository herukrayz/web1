...
if(isset($_POST['simpan'])){
	...
	$id = $_POST['id'];
	$jml = $_POST['jml'];
	mysqli_query($con,"UPDATE keluar SET tgl='".$_POST['tgl']."' WHERE no='".$no."'");
	foreach($id as $index => $value){
    mysqli_query($con,"UPDATE barang as b INNER JOIN keluar_detail AS m ON b.kd = m.kd SET b.jml = b.jml + (".$jml[$index]." - m.jml), m.jml='".$jml[$index]."' WHERE m.id='".$value."'");
	}
...
$query1 = mysqli_query($con, "SELECT * FROM keluar where no='".$no."'");
$row1 = mysqli_fetch_array($query1, MYSQLI_NUM);
$query2 = mysqli_query($con, "SELECT keluar_detail.kd, barang.nm, keluar_detail.jml, barang.stn, keluar_detail.id from keluar_detail inner join barang on keluar_detail.kd=barang.kd WHERE keluar_detail.no='".$no."'");
?>
  <div class="content-wrapper">
    <div class="container-fluid">
      <ol class="breadcrumb">
        <li class="breadcrumb-item active">Edit Transaksi suk</li>
      </ol>
      <div class="row">
